...
1. Update changelog and version
   ...
   - Run `scripts/generate-version.sh` (refreshes `Sources/apple_reminder_cli/Version.swift` + embedded Info.plist).
...
3. Build, sign, and notarize (local)
   ...
   - `scripts/sign-and-notarize.sh` (outputs `/tmp/apple_reminder_cli-macos.zip` by default).
4. Tag, push, and publish
   ...
   - Create GitHub release:
     ```sh
     gh release create v0.1.0 /tmp/apple_reminder_cli-macos.zip -t "v0.1.0" -F /tmp/release-notes.txt
     ```
5. Homebrew tap
   - Update `../homebrew-tap/Formula/apple_reminder_cli.rb` to point at the GitHub release asset.
