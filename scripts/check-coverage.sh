...
CACHE_PATH="${HOME}/Library/Caches/apple_reminder_cli/swiftpm"
...
REPORT_JSON="$(
  find "${COVERAGE_BUILD_PATH}" -type f -path "*debug/codecov/apple_reminder_cli.json" -print0 2>/dev/null \
    | xargs -0 ls -t 2>/dev/null \
    | head -n 1
)"

if [ -z "${REPORT_JSON}" ] || [ ! -f "${REPORT_JSON}" ]; then
  echo "ERROR: Coverage report not found (expected .build/**/debug/codecov/apple_reminder_cli.json)." >&2
  exit 1
fi
